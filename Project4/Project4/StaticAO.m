...
function handles = StaticAO(handles)
...
global Designed_period;
Designed_period = handles.Designed_period ;
global Points_Per_Period
Points_Per_Period = handles.Points_Per_Period;
oneWavePointCount = int32(Points_Per_Period);
global i ;
global temp_i;
global temp_q;
global q;
...
q=temp_q;
f = handles.f;
per = double(1/f)/Points_Per_Period;
...
try
    ...
    errorCode = GenerateWaveform(instantAoCtrl, channelStart, ...
        ...
        H5T.enum_nameof(WaveStyle, int32(wavestyle)));
    ...
    global t_AO;
    t_AO = timer('TimerFcn',{@TimerCallback, instantAoCtrl, ...
        oneWavePointCount, scaleData, scaledWaveForm, channelStart, ...
        channelCount},'busymode','queue', 'period', per, 'executionmode', 'fixedrate', ...
        'StartDelay', 1);
    start(t_AO);
...
%if isempty(Designed_period)
  %  disp('0')

%else
%disp(Designed_period);
%end
global Designed_period;
global Points_Per_Period;
global i;
...
if i>=Points_Per_Period;
    i=i-(Points_Per_Period-1);
    q=q+1;
end
...
if j <= (channelCount - 1)
    ...
    value1 =  scaledWaveForm.Get((channelCount * i + j));
    %  display(value1);
    ...
    x=[i-1+(Points_Per_Period-1)*q,i+(Points_Per_Period-1)*q];
    y=[p,value1];
...
if (Designed_period>0)

if(x(2)==  Points_Per_Period*Designed_period)
    clear functions;
    stop(obj);
    delete(obj);
end
